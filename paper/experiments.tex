...
We have used a single polygon dataset, the area information dataset, published by the Danish government. This dataset contains 30 thousand high-fidelity administrative protection zone polygons, and ranges from small polygons the size of buildings to large polygons the size of entire regions. The biggest polygon contains by 36 thousand points.

\marcos{Cite sources for datasets above.}
...
\begin{table}[htdp]
\caption{Results for CVL on point datasets}
\begin{center}
\begin{tabular}{|c|c|c|c|c|}
\hline
\textbf{Dataset} & \textbf{Constraint} & \textbf{Solver} & \textbf{Time} & \textbf{Opt. ratio}\\ 
\hline
Airports (7K)  & Proximity  & SGA& 1000 s & 1.0 \\
Airports (7K)  & Proximity & LPGA & 1000 s & 1.0 \\
Tourism (500K) & Proximity & SGA & 1000 s & 1.0 \\
Tourism (500K) & Proximity & LPGA & 1000 s & 1.0 \\
Airports (7K) & Visibility & SGA & 1000 s & 1.0 \\
Airports (7K) & Visibility & LPGA & 1000 s & 1.0 \\
Tourism (500K) & Visibility & SGA & 1000 s & 1.0 \\
Tourism (500K) & Visibility & LPGA & 1000 s & 1.0 \\
\hline
\end{tabular}
\end{center}
...
LPGA + Proximity + Points does not scale. We have excluded these from the test.

\subsection{Complex Shape Data}
\label{sec:exp:complex:shapes}

\begin{table}[htdp]
\caption{Results for CVL on complex datasets}
\begin{center}
\begin{tabular}{|c|c|c|c|c|}
\hline
\textbf{Dataset} & \textbf{Constraint} & \textbf{Solver} & \textbf{Time} & \textbf{Opt. ratio}\\ 
\hline
Rivers (4K)  & Proximity  & SGA& 1000 s & 1.0 \\
Rivers (4K)  & Proximity & LPGA & 1000 s & 1.0 \\
Zones (30K) & Proximity & SGA & --- & --- \\
Zones (30K) & Proximity & LPGA & --- & --- \\
Rivers (4K) & Visibility & SGA & 1000 s & 1.0 \\
Rivers (4K) & Visibility & LPGA & 1000 s & 1.0 \\
Zones (30K) & Visibility & SGA & 1000 s & 1.0 \\
Zones (30K) & Visibility & LPGA & 1000 s & 1.0 \\
\hline
\end{tabular}
\end{center}
\label{tab:results:overview}
