...
\frame
{
  \frametitle{Data visualization w/o generalization}
  \begin{itemize}
  \item \textbf{Example}: Restaurant records (points) stored in a database
  \item We want a nice restaurant map of Z\"{u}rich.
...
\frame[t]
{
  \frametitle{Data Visualization with Generalization}
  \emph{What we want}:
\begin{columns}[t]
	\begin{column}[l]{6cm}
		\begin{itemize}[<+->]
			\item Records should appear \emph{gradually} when zooming in
			\item \emph{Information density} should be approximately constant on all zoom-levels
			\item We need a way to prioritize ``important'' records
			\item Items should \emph{remain visible} when zooming in
		\end{itemize}
	\end{column}
	\begin{column}[l]{6cm}
    \begin{figure}
...
\frame
{
  ...
  \begin{itemize}[<+->]
  \item Given that a user wants this map...
  \item How is the desired outcome \emph{specified}?
  \item We could use a \emph{declarative programming language}...\\
  (specify \emph{what} you want, not \emph{how} it is produced)
  \item SQL is a declarative language that can transform spatial data inside a spatial database
  \item But maybe we can design a language for generalization that is easier to use?
  \end{itemize}
}
...
\begin{frame}[fragile,t]
  ...
  \begin{description}[<+->]
  \item[``I want to generalize data in a table called \texttt{restaurants}'']:
\begin{lstlisting}
GENERALIZE restaurants 
\end{lstlisting}
  \item[``Store the result in a new table called \texttt{restaurants2}'']:
\begin{lstlisting}
TO restaurants2
\end{lstlisting}
  \item[``My map will have 20 zoom levels'']:
\begin{lstlisting}
AT 20 ZOOM LEVELS
\end{lstlisting}
  \item[``Use the \texttt{star\_rating} to measure importance of restaurants'']:
\begin{lstlisting}
WEIGH BY star_rating
\end{lstlisting}
  \item[``At least 10 pixels between objects and max 64 objects per tile'']:
\begin{lstlisting}    
SUBJECT TO proximity 10
AND visibility 64
\end{lstlisting}    
  \end{description}
\end{frame}
...
\end{lstlisting}    
\begin{enumerate}
\item \texttt{restaurants2}  is a copy of the input table with an additional column computed: \texttt{min\_zoom} \emph{(explained soon)}
\item New constraints are defined using the \texttt{CREATE~CONSTRAINT} statement \emph{(explained soon)}
\end{enumerate}
