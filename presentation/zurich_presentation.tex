...
\frame[t]
{
  \frametitle{What a good (thematic) map looks like}
\begin{columns}[t]
	\begin{column}[l]{6cm}
		\begin{itemize}[<+->]
...
\item \textbf{Example}: A conflict set under the proximity constraint is a pair of records (that are too close). One must be deleted.
\end{itemize}
\end{frame}
...
\frame
{
  \frametitle{But first some definitions}
  \begin{itemize}
