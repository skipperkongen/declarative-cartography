...
\frame[t]
{
  \frametitle{Situation}
	\begin{itemize}[<+->]
		...
		\item After processing the data is put back into the database
		\item Treats the database as ``dumb'' storage
...
\frame[t]
{
  \frametitle{Why not use the database itself?}
	\begin{itemize}[<+->]
		\item Why not state goals for generalization in a ``high-level'' language?
		\item Translate high-level program to low-level database program
		\item Moving \textbf{code-to-data} instead of data-to-code
		\item Use the database as runtime for generalization
...
\frame[t]
{
  \frametitle{What we mean by generalization}
	\begin{itemize}[<+->]
		...
		\item \textbf{Output (generalized)} : input table extended with \texttt{MinZoom} column
...
\frame{
	...
	\vspace{0.5em}
	\begin{itemize}[<+->]
	\item Constraints defined by defining ``conflicts''
	\item In this example, a conflict is a pair of records that are ``too close''
	\item Resolve a conflict by ``deleting'' one or the other record
	\vspace{0.5em}
	\end{itemize}
	\begin{center}
  		\includegraphics[scale=0.35]{figs/cvl-proximity-conflicts-2.pdf}
  	\end{center}
...
\frame[t]
{
	\frametitle{Correspondence to bi-partite graphs}
	\begin{itemize}[<+->]
	...
	\begin{center}
	\begin{figure}
  		\includegraphics[scale=0.8]{figs/cvl-bipartite-graph.pdf}
	\caption{Correspondence between conflicts and bi-partite graphs}
	\end{figure}
  	\end{center}
	\end{itemize}
}
...
\frame[t]
{
	\frametitle{Compute solution on bi-partite graphs}
	\begin{itemize}[<+->]
	...
	\item \textbf{Problem}: Pick records with minimum total weight on left \\s.t. conflicts on right resolved
	\end{itemize}
	\vspace{1em}
	\begin{center}
	\begin{figure}
